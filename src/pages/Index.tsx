  ...
  return (
    <div className="min-h-screen font-montserrat" style={{ background: "#060d1f" }}>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-20">
          <button onClick={() => scrollTo("home")} className="flex flex-col items-start">
            <span className="font-cormorant text-2xl tracking-widest leading-none" style={{ color: "#d4a843" }}>
              НОВЫЙ ВЗГЛЯД
            </span>
            <span className="text-[10px] tracking-[0.25em] mt-0.5 font-montserrat font-light" style={{ color: "rgba(240,208,128,0.5)" }}>
              ОФТАЛЬМОЛОГИЯ
            </span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <button
                ...
                style={{ color: activeSection === link.id ? "#d4a843" : "rgba(255,255,255,0.65)" }}
              >
                {link.label}
                <span
                  className="absolute bottom-0 left-0 h-px transition-all duration-500"
                  style={{ width: activeSection === link.id ? "100%" : "0", background: "#d4a843" }}
                />
              </button>
            ))}
          </div>

          <button className="md:hidden p-2" onClick={() => setMenuOpen(!menuOpen)} style={{ color: "#d4a843" }}>
          ...
          <div className="md:hidden px-6 pb-6 flex flex-col gap-4" style={{ background: "rgba(6,13,31,0.98)" }}>
            {navLinks.map((link) => (
              <button
                ...
                style={{ color: activeSection === link.id ? "#d4a843" : "rgba(255,255,255,0.7)", borderColor: "rgba(212,168,67,0.15)" }}
              >
                {link.label}
              </button>
            ))}
      ...
      <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(212,168,67,0.35), transparent)" }} />

        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          <p className="animate-fade-up opacity-0 text-[10px] md:text-xs tracking-[0.45em] mb-8 font-light" style={{ color: "rgba(240,208,128,0.55)" }}>
            ОФТАЛЬМОЛОГИЧЕСКАЯ КЛИНИКА
          </p>

          <h1
            className="animate-fade-up-delay opacity-0 font-cormorant font-light leading-[0.88] mb-8"
            style={{ fontSize: "clamp(4rem, 11vw, 9rem)", color: "#ffffff" }}
          >
            Новый<br />
            <span style={{ color: "#d4a843", fontStyle: "italic" }}>Взгляд</span>
          </h1>

          <div
            className="mx-auto my-8 h-px animate-line-grow opacity-0"
            style={{ maxWidth: 100, background: "linear-gradient(90deg, transparent, #d4a843, transparent)" }}
          />

          <p className="animate-fade-up-delay2 opacity-0 font-cormorant font-light text-xl md:text-2xl leading-relaxed max-w-xl mx-auto" style={{ color: "rgba(255,255,255,0.5)" }}>
          ...
          <div className="animate-fade-up-delay2 opacity-0 mt-14 flex flex-col sm:flex-row gap-4 justify-center">
            <button
              ...
              style={{ background: "linear-gradient(135deg, #b07828, #d4a843)", color: "#03060f" }}
            >
              ЗАПИСАТЬСЯ НА ПРИЁМ
            </button>
            <button
              ...
              style={{ border: "1px solid rgba(212,168,67,0.4)", color: "rgba(240,208,128,0.8)", background: "transparent" }}
            >
              О КЛИНИКЕ
            </button>
          </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce">
          <div className="h-10 w-px" style={{ background: "rgba(212,168,67,0.3)" }} />
          <Icon name="ChevronDown" size={16} style={{ color: "rgba(212,168,67,0.35)" }} />
        </div>
      </section>
      ...
      <section id="about" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={aboutSection.ref} className="grid md:grid-cols-2 gap-20 items-center">
          ...
          >
            <p className="text-[10px] tracking-[0.35em] mb-6" style={{ color: "rgba(212,168,67,0.6)" }}>О КЛИНИКЕ</p>
            <h2
              className="font-cormorant font-light leading-tight mb-8"
              style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)", color: "#ffffff" }}
            >
              Точность взгляда —<br />
              <span style={{ color: "#d4a843", fontStyle: "italic" }}>наша профессия</span>
            </h2>
            <div className="h-px w-16 mb-8" style={{ background: "#d4a843" }} />
          ...
          <div
            ...
            style={{ opacity: aboutSection.inView ? 1 : 0, transform: aboutSection.inView ? "translateX(0)" : "translateX(50px)" }}
          >
            <div className="absolute -inset-3" style={{ border: "1px solid rgba(212,168,67,0.12)" }} />
            <img
              src={CLINIC_IMAGE}
            ...
            />
            <div className="absolute -bottom-4 -right-4 w-24 h-24 z-0" style={{ border: "1px solid rgba(212,168,67,0.18)" }} />
          </div>
        </div>
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                borderLeft: i > 0 ? "1px solid rgba(212,168,67,0.1)" : "none",
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center mb-6 transition-all duration-300 group-hover:scale-110" style={{ border: "1px solid rgba(212,168,67,0.3)" }}>
                <Icon name={item.icon} size={20} style={{ color: "#d4a843" }} />
              </div>
              <h3 className="font-cormorant text-xl font-light mb-3" style={{ color: "#ffffff" }}>{item.title}</h3>
      ...
      {/* Divider */}
      <div className="max-w-7xl mx-auto px-12 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(212,168,67,0.18), transparent)" }} />
      ...
      <section id="contacts" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={contactsSection.ref}>
          ...
          >
            <p className="text-[10px] tracking-[0.35em] mb-6" style={{ color: "rgba(212,168,67,0.6)" }}>СВЯЗЬ С НАМИ</p>
            <h2 className="font-cormorant font-light" style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)", color: "#ffffff" }}>
              Контакты
            </h2>
            <div className="h-px mx-auto mt-6" style={{ width: 60, background: "#d4a843" }} />
          </div>
          ...
          >
            {[
              ...
              >
                <div className="w-14 h-14 flex items-center justify-center mb-6 rounded-full" style={{ border: "1px solid rgba(212,168,67,0.3)" }}>
                  <Icon name={item.icon} size={20} style={{ color: "#d4a843" }} />
                </div>
                <h3 className="font-cormorant text-lg font-light mb-4" style={{ color: "rgba(240,208,128,0.8)" }}>{item.title}</h3>
      ...
      {/* FOOTER */}
      <footer className="py-10 text-center" style={{ borderTop: "1px solid rgba(212,168,67,0.1)" }}>
        <p className="font-cormorant text-2xl tracking-widest mb-2" style={{ color: "#d4a843" }}>НОВЫЙ ВЗГЛЯД</p>
