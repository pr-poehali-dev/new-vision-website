    ...
    <div className="min-h-screen font-montserrat" style={{ background: "#f5f3f8" }}>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-20">
          <button onClick={() => scrollTo("home")} className="flex flex-col items-start">
            ...
            <span className="text-[10px] tracking-[0.25em] mt-0.5 font-montserrat font-light" style={{ color: "#1e2d45" }}>
              ОФТАЛЬМОЛОГИЯ
            </span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <button
                ...
                style={{ color: activeSection === link.id ? "#1a1a2e" : "#1e2d45" }}
              >
                {link.label}
                <span
          ...
          <div className="md:hidden px-6 pb-6 flex flex-col gap-4" style={{ background: "rgba(245,243,248,0.98)" }}>
            {navLinks.map((link) => (
              <button
                ...
                style={{ color: activeSection === link.id ? "#1a1a2e" : "#1e2d45", borderColor: "rgba(61,79,107,0.2)" }}
              >
                {link.label}
              </button>
            ))}
      ...
      <section id="home" className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="relative z-10 flex-1 px-10 md:px-20 max-w-3xl">
          <p className="animate-fade-up opacity-0 text-[10px] md:text-xs tracking-[0.45em] mb-8 font-light" style={{ color: "#1e2d45" }}>
          ...
          <p className="animate-fade-up-delay2 opacity-0 font-roboto font-light text-lg md:text-xl leading-relaxed max-w-md mb-10" style={{ color: "#1f2937" }}>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce">
          ...
          <Icon name="ChevronDown" size={16} style={{ color: "rgba(30,45,69,0.6)" }} />
        </div>
      </section>
      ...
      <section id="about" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={aboutSection.ref} className="grid md:grid-cols-2 gap-20 items-center">
          ...
          >
            <p className="text-[10px] tracking-[0.35em] mb-6" style={{ color: "#1e2d45" }}>О КЛИНИКЕ</p>
            <h2
              className="font-roboto font-light leading-tight mb-8"
              style={{ fontSize: "clamp(2.5rem, 5vw, 4rem)", color: "#1a1a2e" }}
            >
              Точность взгляда —<br />
              <span style={{ color: "#1e2d45", fontStyle: "italic" }}>наша профессия</span>
            </h2>
            <div className="h-px w-16 mb-8" style={{ background: "#6b7a99" }} />
            <p className="font-light text-base leading-8 mb-6" style={{ color: "#1f2937" }}>
            ...
            <p className="font-light text-base leading-8" style={{ color: "#1f2937" }}>
        ...
        <p className="mt-20 font-roboto font-light text-center" style={{ fontSize: "clamp(1.1rem, 2.5vw, 1.6rem)", color: "#1e2d45", fontStyle: "italic" }}>
      ...
      <section id="contacts" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        <div ref={contactsSection.ref}>
          ...
          >
            <p className="text-[10px] tracking-[0.35em] mb-6" style={{ color: "#1e2d45" }}>СВЯЗЬ С НАМИ</p>
            ...
            {[
              ...
              >
                ...
                {item.lines.map((line, j) => (
                  item.href[j] ? (
                    <a key={j} href={item.href[j]!} className="font-light text-sm" style={{ color: j === 0 ? "#1a1a2e" : "#1f2937", marginBottom: j === 0 ? 4 : 0, display: "block" }}>
                      {line}
                    </a>
                  ) : (
                    <p key={j} className="font-light text-sm" style={{ color: j === 0 ? "#1a1a2e" : "#1f2937", marginBottom: j === 0 ? 4 : 0 }}>
                      {line}
                    </p>
                  )
                ))}
